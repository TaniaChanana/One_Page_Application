...
public class Student extends JFrame {
	...
	JLabel jwelcome, jlogin, jpass, jstudentname, jpreaddress, jpermaaddress, jfathername, jgender, jcourse, jyear,
			jlabel, jdob, jfatheroccupation, jphone, jlabel111;
	JLabel jmstudentname, jmpreaddress, jmpermaaddress, jmfathername, jmgender, jmcourse, jmyear, jmlabel, jmdob,
			jmfatheroccupation, jmphone;
	...
	int /* ctr_val = 0, */selstudentID = 0, selstudentname = 0, selstudentpreadd = 0, selstudentpermaadd, selfathername = 0,
			selgender = 0, seldob = 0;
...
//-----------Starting with the displayPanel1() method to create the Login panel
	...
	// ------------------------------------------------------------ Ending with
	// displayPanel1() method -----------
	// ============================================================ Starting with
	// displayPanel2() method ===========
...
//		accessStudName();
	...
	void verifyLogin() // Verifing Login for username 'administrator' and password 'password'
	{
		try {
			...
			while (rs.next()) {
				if (rs.getString(1).equalsIgnoreCase(val1) && rs.getString(2).equalsIgnoreCase(val2)) {
					showResults(rs);
				}else {
					showResults(null);
				}
	...
	// --------------------------------------------Ending with verifyLogin()
	// method-------

	// --------------------------------------------Starting with showResults()
	// method------
	...
	// ------------------------------------------------------------------- Ending
	// with showResults() method -------

	// =================================================================== Starting
	// with addRecord() method====
	void addRecord() {
		try {
			...
			query3 = "INSERT INTO student_details (name,preaddress,permaaddress,fathername,dateofbirth,fatheroccupation,phone,gender,year,course,dmc,degree,charcerti,ncccerti) VALUES ('"
					+ tsname.getText() + "','" + tspreaddress.getText() + "','" + tspermaaddress.getText() + "','"
					+ tsfathername.getText() + "','" + tsdob.getText() + "','" + foccupation + "','" + tsphone.getText()
					+ "','" + gender + "','" + year + "','" + course + "'," + dmcsel + "," + degreesel + "," + charcfsel
					+ "," + ncccfsel + ")";
...
//			data[0] = reg_no;
			...
			tspermaaddress.setText("");
			tsfatheroccupation.setText("");
	...
	void accessModifiedDatabase(int nsel) {
		try {
			...
			Statement st = getStatement();
			ResultSet result = st.executeQuery(query4);
				if (result != null) {
	...
	// ------------------------------------------------------------------- Ending
	// with showModifiedResults() -------
	// =============================================================== Starting with
	// accessStudName() ========

	private void intializeAllTextField() {
		tmname = new JTextField(20);
		...
		m_male = new JRadioButton("Male", true);
		m_female = new JRadioButton("FeMale", true);
		mcb_course = new JComboBox ();
		// Adding the combo box elements to combo box
		mcb_course.addItem("BSc(Comp.Sc.)");
		mcb_course.addItem("BCA");
		...
		mcb_year.addItem(" III Year ");
		mdmc = new JCheckBox();
		mdegree = new JCheckBox();
		mcharcerti = new JCheckBox();
		mncc = new JCheckBox();
	...
	{
		try {
			query7 = "UPDATE student_details SET name='" + tmname.getText() + "', preaddress='" + tmpreaddress.getText()
					+ "', permaaddress='" + tmpermaaddress.getText() + "', fathername='" + tmfathername.getText()
					+ "', dateofbirth='" + tmdob.getText() + "', fatheroccupation='" + foccupation + "', phone='"
					+ tmphone.getText() + "', gender='" + gender + "', year='" + year + "', course='" + course
					+ "', dmc=" + dmcsel + ", degree=" + degreesel + ", charcerti=" + charcfsel + ", ncccerti="
					+ ncccfsel + " WHERE registration_no=" + std_id;
	...
	class ButtonHandler implements ActionListener {
		public void actionPerformed(ActionEvent ev) {
			...
			{
				String [] itemValue = selname.split(":");
				int student_id = Integer.parseInt(itemValue[0]);
				updateRecord(student_id, selname);
//				int x = ctr_val;

				/*
				 * for (int y = x; y > 1; y--) { cb_sname.removeItemAt(y);
				 * 
				 * }
				 */
				cb_sname.addItem("Select Student Name");
	...
	class ComboBoxListener implements ItemListener {
		...
		public void itemStateChanged(ItemEvent e) {
			...
			if (e.getSource().equals(cb_sname)) {
				...
				accessModifiedDatabase(Integer.parseInt(itemValue[0]));
			}
